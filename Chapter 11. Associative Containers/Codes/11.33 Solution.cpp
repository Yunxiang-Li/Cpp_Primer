 ...
 */
std::map<std::string, std::string> buildMap(std::ifstream& ifs_map) {
 ...
 */
const std::string& transform(const std::string& word, const std::map<std::string, std::string>& map) {
  std::map<std::string, std::string>::const_iterator iter = map.find(word);
  if (iter != map.cend())
    return iter -> second;
  else
    return word;
...
void word_transform(std::ifstream& ifs_map, std::ifstream& ifs_content) {
  std::map<std::string, std::string> word_transform_map{buildMap(ifs_map)};
  std::string line;
