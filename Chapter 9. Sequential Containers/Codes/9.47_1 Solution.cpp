...
int main()
{
  ...
  for (std::string::size_type pos = 0; (pos = str.find_first_of(numStr, pos)) != std::string::npos; ++pos)
    std::cout << "Find one numeric character: " << str[pos] << '\n';

  std::cout << "Next find each alphabetic character" << '\n';
  for (std::string::size_type pos = 0; (pos = str.find_first_of(alphabetStr, pos)) != std::string::npos; ++pos)
