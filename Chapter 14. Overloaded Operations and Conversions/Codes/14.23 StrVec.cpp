...
StrVec& StrVec::operator=(std::initializer_list<std::string> initializer_list) {
    std::pair<std::string*, std::string*> newPair = m_alloc_n_copy(initializer_list.begin(), initializer_list.end());
    m_free();
    m_elements = newPair.first;
    m_first_free = newPair.second;
    m_cap = newPair.second;
...
std::pair<std::string *, std::string *> StrVec::m_alloc_n_copy(const std::string* strOne, const std::string* strTwo) {
    std::string* beginPtr = alloc.allocate(strTwo - strOne, nullptr);
